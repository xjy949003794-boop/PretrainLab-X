...
PretrainLab-X 不是单独的一份模型代码，而是一套从模型结构、数据处理、训练引擎、checkpoint 恢复、训练监控，到长程预训练、共享留出评估、统计检验和文件审计的完整闭环。

项目里最有价值的一次推进，来自第一次 336M 长跑暴露出的数据问题：虽然训练循环一共处理了约 5 亿个 token position，但底层训练切片只有 990 万 token，因此被反复回放。这个结果没有直接被包装成“500M 数据训练”，而是反过来推动了数据管线重建：重新准备约 5.05 亿 token 的 FineWeb-Edu 训练语料，在相同 336M 模型规模和基本一致的 token 预算下，再完成一次不回放长跑。

随后，两份冻结 checkpoint 被放到同一套共享留出协议上考试。由于差距非常大，后续没有直接接受这个结果，而是专门增加了一层反向验证：检查 checkpoint 是否损坏、模型配置是否一致、随机模型会不会也得到“好成绩”、Day 5 是否只是记住训练切片、Day 6 的训练侧和留出侧是否一致、BF16 是否造成数值假象、留出数据是否存在可检测重叠，并最终做配对 Bootstrap 与哈希审计。
